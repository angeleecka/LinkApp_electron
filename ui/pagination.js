...
function onGlobalKeydown(e) {
  ...
  if (
    ...
    if (j) {
      e.preventDefault();
      j.focus();
      // опционально: открыть выпадашку на некоторых браузерах
      try {
        j.size = Math.min(12, j.options.length || 12);
        setTimeout(() => (j.size = 0), 0);
      } catch {}
    }
  }
}
...
export function renderPagination() {
  ...
  addPageBtn.className = "pagination-btn add-page-button";
